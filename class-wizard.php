...
class Wizard {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-logger.php';
		$this->logger = Wizard_Logger::get_instance();
		...
		add_action( 'wp_ajax_wizard_activate_license', array( $this, '_ajax_activate_license' ), 10, 0 );
	...
	function required_classes() {
		...
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-downloader.php';

		$this->importer = new ProteusThemes\WPContentImporter2\Importer( array( 'fetch_attachments' => true ), $this->logger );

		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-widget-importer.php';

		if ( ! class_exists( 'WP_Customize_Setting' ) ) {
			require_once ABSPATH . 'wp-includes/class-wp-customize-setting.php';
		}

		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-customizer-option.php';
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-customizer-importer.php';
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-redux-importer.php';
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-wizard-hooks.php';

		$this->hooks = new Wizard_Hooks();
	...
	/**
	 * Give the user the ability to ignore wizard.
	 */
	...
	public function admin_page() {
		...
		// Enqueue styles.
		wp_enqueue_style( 'wizard', trailingslashit( $this->base_url ) . $this->directory . '/assets/css/wizard' . $suffix . '.css', array( 'wp-admin' ), MERLIN_VERSION );

		// Enqueue javascript.
		wp_enqueue_script( 'wizard', trailingslashit( $this->base_url ) . $this->directory . '/assets/js/wizard' . $suffix . '.js', array( 'jquery-core' ), MERLIN_VERSION );
	...
	protected function license() {
		...
		<div class="wizard__content--transition">
			...
			<?php if ( ! $is_theme_registered ) : ?>
				<div class="wizard__content--license-key">
					<label for="license-key"><?php echo esc_html( $label ); ?></label>

					<div class="wizard__content--license-key-wrapper">
		...
		<footer class="wizard__content__footer <?php echo esc_attr( $is_theme_registered_class ); ?>">
			...
			<?php if ( ! $is_theme_registered ) : ?>
				...
				<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="wizard__button wizard__button--next button-next js-wizard-license-activate-button" data-callback="activate_license">
	...
	private function is_theme_registered() {
		...
		return apply_filters( 'wizard_is_theme_registered', $is_registered );
	}
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'wizard_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
		...
		if ( ! has_filter( 'wizard_ajax_activate_license' ) ) {
			$result = $this->edd_activate_license( $license_key );
		} else {
			$result = apply_filters( 'wizard_ajax_activate_license', $license_key );
		}
	...
	public function generate_child_functions_php( $slug ) {
		...
		$output = "
			 ...
			 * This child theme was generated by wizard.
			 */
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			...
			* Description: This is a child theme of {$parent}, generated by wizard.
			* Author: {$author}
			* Template: {$slug}
			* Version: {$version}
			*/\n
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				...
				'installing'       => esc_html__( 'Installing', '@@textdomain' ),
				'success'          => esc_html__( 'Success', '@@textdomain' ),
				'install_callback' => array( 'Wizard_Widget_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				...
				'installing'       => esc_html__( 'Installing', '@@textdomain' ),
				'success'          => esc_html__( 'Success', '@@textdomain' ),
				'install_callback' => array( 'Wizard_Customizer_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				...
				'installing'       => esc_html__( 'Installing', '@@textdomain' ),
				'success'          => esc_html__( 'Success', '@@textdomain' ),
				'install_callback' => array( 'Wizard_Redux_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		$downloader = new Wizard_Downloader();
