...
class Wizard_Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		if ( is_wp_error( $content ) ) {
			Wizard_Logger::get_instance()->error( $content->get_error_message(), array( 'url' => $url, 'filename' => $filename ) );
		...
		Wizard_Logger::get_instance()->error( __( 'The file was not able to save to disk, while trying to download it', '@@textdomain' ), array( 'url' => $url, 'filename' => $filename ) );
